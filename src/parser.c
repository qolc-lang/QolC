	...
	while (current != NULL) {
		...
		if (isNumberType(current->type) == 1) {
			...
			if (strcmp(theStack[top], "print") == 0) {
				...
				pop(&top);
				theStack[0][top] = '\0';
				BuilAddDeclaration(temp, temp2, commandNode);
			}
			else if (strcmp(theStack[top], "-") == 0) {
				doneFlag = 1;
				theStack[0][top] = '\0';
				...
				theStack[0][top] = '\0';
				pop(&top);
				BuilAddAssignmentExpression(temp, temp2, commandNode);
				notTheEndFlag = 0;
			}
			else if (strcmp(theStack[top], "-=") == 0) {
				doneFlag = 1;
		...
		if (strcmp(current->type, "keyword") == 0) {
			...
			if (strcmp(current->value, "null") == 0) {
				...
				if (strcmp(theStack[top], "float") == 0) {
					...
					pop(&top);
					theStack[0][top] = '\0';
					BuildIntPointerDeclaration(current->value, commandNode, temp);
				}
				else if (strcmp(theStack[top], "char") == 0) {
					doneFlag = 1;
					theStack[0][top] = '\0';
					...
					pop(&top);
					theStack[0][top] = '\0';
					BuildCharPointerDeclaration(current->value, commandNode, temp);
				}
				else if (strcmp(theStack[top], "string") == 0) {
					doneFlag = 1;
					theStack[0][top] = '\0';
